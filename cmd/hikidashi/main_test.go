...
// invoke は run を呼び、終了コードと stdout・stderr の中身を返す。
func invoke(cmds []command, args ...string) (code int, stdout, stderr string) {
	var out, errOut bytes.Buffer
	code = run(cmds, args, &out, &errOut)
...
func TestRunHelpFailsWhenStdoutIsNotWritable(t *testing.T) {
	...
	code := run(nil, []string{"help"}, failingWriter{}, &errOut)
...
func TestRunDispatchesToCommand(t *testing.T) {
	var gotArgs []string
	cmds := []command{
		{name: "other", run: func([]string, io.Writer, io.Writer) int {
			t.Error("other command must not run")
			return 1
		}},
		{name: "status", run: func(args []string, stdout, stderr io.Writer) int {
			gotArgs = args
	...
	code, stdout, stderr := invoke(cmds, "status", "-x", "y")
