...
`tmux.conf` に、プロジェクトの一覧をポップアップで開くキーバインドと、入力待ちの件数をステータスバーに出す設定を書く。
...
プレビュー（`hikidashi show <プロジェクト>`）は一覧の下に全幅で出る。
...
幅が 120 桁以上あってセッションがあれば、左にセッション、右にプロジェクトと備忘録を並べて出す。
